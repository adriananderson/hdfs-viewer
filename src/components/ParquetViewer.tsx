...
function DataView({ columns, rows }: { columns: ParquetColumn[]; rows: Record<string, unknown>[] }): JSX.Element {
  if (rows.length === 0) {
    return <div className="flex-1 flex items-center justify-center text-gray-500 text-sm">No rows</div>
  }
      ...
      <table className="text-xs border-collapse min-w-full">
        <thead className="sticky top-0 z-10">
          <tr className="bg-gray-800 border-b border-gray-700">
            ...
            {columns.map((col) => (
              <th key={col.name} className="text-left py-2 px-3 text-gray-300 font-medium whitespace-nowrap border-r border-gray-700 last:border-r-0">
        ...
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className={`border-b border-gray-800 hover:bg-gray-800/60 ${ri % 2 === 0 ? '' : 'bg-gray-850'}`}>
              ...
              {columns.map((col) => {
                ...
                return (
                  <td
                    key={col.name}
                    className={`py-1.5 px-3 border-r border-gray-800 last:border-r-0 max-w-xs ${isNull ? 'text-gray-600 italic' : 'text-gray-200'}`}
                    title={display}
                  >
...
export function ParquetViewer(): JSX.Element {
  ...
  const loadingParquet = useAppStore((s) => s.loadingParquet)
  const activeTab = useAppStore((s) => s.activeTab)
  const setActiveTab = useAppStore((s) => s.setActiveTab)

  if (loadingParquet) {
    return <div className="flex-1 flex items-center justify-center text-gray-500 text-sm">Reading file…</div>
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-gray-900">
        ...
        <div>
          ...
          <div className="text-xs text-gray-400 mt-0.5">
            ...
            {loadedRows < totalRows && (
              <span className="text-yellow-500 ml-1">(showing first {loadedRows.toLocaleString()})</span>
            )}
          </div>
        </div>
          ...
          {(['data', 'schema'] as const).map((tab) => (
            <button
              ...
              className={`px-3 py-1.5 transition-colors ${
                activeTab === tab ? 'bg-blue-600 text-white font-medium' : 'bg-gray-800 text-gray-400 hover:text-gray-200'
              }`}
            >
      ...
      {activeTab === 'schema' ? <SchemaView columns={columns} /> : <DataView columns={columns} rows={rows} />}
    </div>
  )
}
